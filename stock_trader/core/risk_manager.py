...
class RiskManager:
    ...
    def __init__(self, trailing_min_drop=3.0, trailing_max_drop=10.0, chandelier_atr_mult=2.5, market_regime="NEUTRAL"):
    ...
    def calculate_stops(self, atr_pct: float) -> tuple:
        ...
        # 하드 스탑: 초기 진입 시 절대 허용 불가선 (최소 3%, 최대 6% 사이, 기본 1.5 ATR)
        dynamic_hard_stop = -max(3.0, min(6.0, 1.5 * atr))
    ...
    def calculate_size_factor(self, atr_pct: float, base_weight: float = 1.0) -> float:
        ...
        if self.market_regime == "BULL":
            # 대세 상승장: 매수 비중 상향 오버웨이트 제거 (과접합 방지)
            size_factor *= 1.0
        elif self.market_regime == "BEAR":
            # 대세 하락장: 매수 비중 대폭 하향 (방어적 운용)
            size_factor *= 0.2
